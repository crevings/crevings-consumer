...
export const downloadInvoice = (order: Order) => {
  ...
  invoiceWindow.document.write(`
    ...
    <html>
    <head>
      <title>Crevings Tax Invoice - ${orderId}</title>
      <style>
        ...
        .invoice-card { max-width: 650px; margin: 0 auto; border: 1px solid #e2e8f0; padding: 36px; border-radius: 20px; shadow: 0 10px 25px rgba(0,0,0,0.05); }
        ...
        .info-block { background: #f8fafc; p: 14px; padding: 14px; border-radius: 12px; border: 1px solid #f1f5f9; }
            ...
            <div className="space-y-3 bg-white rounded-2xl p-4 border border-slate-100 divide-y divide-slate-100">
              {itemsList.map((item: any, idx: number) => (
                <div key={idx} className="pt-3 first:pt-0 flex items-center justify-between text-sm">
                  ...
                  <span className="font-bold text-slate-900">₹{(item.price || 120) * (item.quantity || 1)}</span>
                </div>
              ))}
            </div>
          </div>
