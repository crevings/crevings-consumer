      ...
      <div className="flex-1 overflow-y-auto no-scrollbar px-5 pt-5 pb-10">
        ...
        {searchQuery.length > 0 ? (
          ...
          <>
            ...
            <div className="mb-10">
              <h2 className="text-[17px] font-bold text-slate-900 mb-4 px-1">Popular cities</h2>
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  { name: 'Chandigarh', icon: HandMetal },
                ].map((city) => (
                  <button 
                    key={city.name}
                    className="bg-[#f8f9fc] rounded-[16px] p-3 flex flex-col items-center justify-center gap-3 aspect-square active:scale-95 transition-transform"
                    onClick={() => {
                       setSearchQuery(city.name);
                    }}
                  >
                    <city.icon className="w-8 h-8 text-[#5b5fd8]" strokeWidth={1} />
                    <span className="text-[14px] font-bold text-slate-900 text-center leading-tight">{city.name}</span>
                  </button>
                ))}
              </div>
            </div>
